...
async def test_run_logs_404_nonexistent_run(untested_authed_client):
    ...
    resp = await client.get(
        "/api/jobs/runs/nonexistent/logs", headers=auth_headers(api_key)
    )
    assert resp.status_code == 404
...
async def test_run_logs_empty_for_new_run(untested_authed_client):
    ...
    resp = await client.get(
        f"/api/jobs/runs/{run_id}/logs", headers=auth_headers(api_key)
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_encryption_password_endpoint_removed(client):
    ...
    resp = await client.get(
        "/api/settings/encryption-password", headers=auth_headers(api_key)
    )
...
async def test_settings_does_not_expose_encryption_password(client):
    ...
    resp = await client.get(
        "/api/settings", headers=auth_headers(api_key)
    )
    assert resp.status_code == 200
    body = resp.json()
    ...
    async with aiosqlite.connect(config.db_path) as db:
        ...
        await db.execute(
            """INSERT INTO storage_targets
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            ("tgt1", "Local", "local", 1, '{"path": "/tmp/bk"}',
             "ok", "2025-01-01", "2025-01-01"),
        )
        await db.commit()

    try:
        resp = await client.post(
            "/api/snapshots/refresh", headers=auth_headers(api_key)
        )
        assert resp.status_code == 200
        body = resp.json()
...
async def test_get_snapshot_detail(untested_authed_client):
    ...
    resp = await client.get(
        f"/api/snapshots/{snap_id}", headers=auth_headers(api_key)
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_get_snapshot_detail_404(untested_authed_client):
    ...
    client, api_key = untested_authed_client
    resp = await client.get(
        "/api/snapshots/nonexistent", headers=auth_headers(api_key)
    )
    assert resp.status_code == 404
...
async def test_get_snapshot_by_full_id(untested_authed_client):
    ...
    resp = await client.get(
        f"/api/snapshots/{full_id}", headers=auth_headers(api_key)
    )
    assert resp.status_code == 200
...
async def test_restore_browse_snapshot_not_found(untested_authed_client):
    ...
    client, api_key = untested_authed_client
    resp = await client.get(
        "/api/restore/browse/nonexistent", headers=auth_headers(api_key)
    )
    assert resp.status_code == 404
    ...
    async with aiosqlite.connect(config.db_path) as db:
        await db.execute(
            """INSERT INTO storage_targets
               (id, name, type, enabled, config, status, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            ("tgt-browse", "BrowseTarget", "local", 1,
             '{"path": "/tmp/bk"}', "ok", "2025-01-01", "2025-01-01"),
        )
        await db.commit()
...
async def test_restore_plan_markdown(untested_authed_client):
    ...
    client, api_key = untested_authed_client
    resp = await client.get(
        "/api/restore/plan", headers=auth_headers(api_key)
    )
    assert resp.status_code == 200
...
async def test_discover_alias_returns_503_without_docker(untested_authed_client):
    ...
    client, api_key = untested_authed_client
    resp = await client.post(
        "/api/discover", headers=auth_headers(api_key)
    )
...
async def test_discover_scan_returns_503_without_docker(untested_authed_client):
    ...
    client, api_key = untested_authed_client
    resp = await client.post(
        "/api/discover/scan", headers=auth_headers(api_key)
    )
    assert resp.status_code == 503
...
async def test_directories_scan(untested_authed_client):
    ...
    client, api_key = untested_authed_client
    resp = await client.post(
        "/api/directories/scan", headers=auth_headers(api_key)
    )
    assert resp.status_code == 200
    body = resp.json()
